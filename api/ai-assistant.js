...
function withTimeout(promise,ms,label){let timer;const timeout=new Promise((_,reject)=>{timer=setTimeout(()=>{const e=new Error(label||'Request timed out');e.code='TIMEOUT';reject(e);},ms);});return Promise.race([promise,timeout]).finally(()=>clearTimeout(timer));}
async function entitlement(sql,u){const rows=await sql`SELECT plan,status,current_period_end AS "currentPeriodEnd" FROM ai_assistant_entitlements WHERE user_id=${u.id} LIMIT 1`;const e=rows[0]||{};const plus=e.plan==='plus'&&['active','trialing'].includes(String(e.status||''))&&(!e.currentPeriodEnd||new Date(e.currentPeriodEnd).getTime()>Date.now());return {plan:plus?'plus':'free',limit:plus?PLUS_LIMIT:FREE_LIMIT};}
...
async function rollover(sql,u){try{const rows=await sql`SELECT credits FROM ai_credit_balances WHERE user_id=${u.id} LIMIT 1`;return Math.max(0,Number(rows[0]?.credits||0));}catch{return 0;}}
...
async function guideContext(req,paths){const contexts=[];const sources=[];for(const raw of paths.slice(0,3)){const path=String(raw||'');if(!/^\/guides\/[a-z0-9-]+\.html$/i.test(path))continue;try{const r=await fetch(`${origin(req)}${path}`,{headers:{'user-agent':'RightsRadarAI/1.0'}});if(!r.ok)continue;const html=await r.text();const title=(html.match(/<h1[^>]*>([\s\S]*?)<\/h1>/i)||[])[1]||path;const text=stripHtml((html.match(/<article[^>]*>([\s\S]*?)<\/article>/i)||[])[1]||html).slice(0,6500);if(text)contexts.push(`VERIFIED RIGHTSRADAR GUIDE: ${stripHtml(title)}\nURL: ${path}\n${text}`);for(const s of sourceLinks(html)){if(!sources.some(x=>x.url===s.url))sources.push(s);}}catch{}}return {contexts,sources};}
...
module.exports=async function handler(req,res){
 ...
 const conn=dbUrl();if(!conn)return send(res,503,{error:'Assistant account database is not connected.',code:'DB_NOT_CONFIGURED'});const sql=neon(conn);
 try{
  ...
  const [ent,used,topupCredits]=await withTimeout(Promise.all([entitlement(sql,u),usage(sql,u),rollover(sql,u)]),8000,'Assistant account status timed out');
  const monthlyRemaining=Math.max(0,ent.limit-used);const totalRemaining=monthlyRemaining+topupCredits;
  if(req.method==='GET')return send(res,200,{ok:true,user:{name:u.name},plan:ent.plan,used,limit:ent.limit,monthlyRemaining,topupCredits,remaining:totalRemaining,aiConfigured:Boolean(process.env.OPENAI_API_KEY)});
  if(!sameOrigin(req))return send(res,403,{error:'Invalid request origin.'});
  if(totalRemaining<=0)return send(res,402,{error:ent.plan==='free'?'Your free monthly AI questions and top-up credits are used. Upgrade to RightsRadar Plus or add more credits.':'Your monthly AI allowance and top-up credits are used. Add more credits to continue.',code:'LIMIT_REACHED',plan:ent.plan,used,limit:ent.limit,topupCredits});
  ...
  let nowUsed=used;let nowTopup=topupCredits;
  if(used<ent.limit){const rows=await sql`INSERT INTO ai_assistant_usage(user_id,period_start,message_count) VALUES(${u.id},date_trunc('month',CURRENT_DATE)::date,1) ON CONFLICT(user_id,period_start) DO UPDATE SET message_count=ai_assistant_usage.message_count+1,updated_at=NOW() RETURNING message_count AS count`;nowUsed=Number(rows[0]?.count||used+1);}else{const rows=await sql`UPDATE ai_credit_balances SET credits=credits-1,updated_at=NOW() WHERE user_id=${u.id} AND credits>0 RETURNING credits`;if(!rows.length)return send(res,409,{error:'Your credit balance changed while this request was running. Please try again.',code:'CREDIT_RACE'});nowTopup=Number(rows[0].credits||0);}
  return send(res,200,{ok:true,answer,sources:grounded.sources.slice(0,6),plan:ent.plan,used:nowUsed,limit:ent.limit,monthlyRemaining:Math.max(0,ent.limit-nowUsed),topupCredits:nowTopup,remaining:Math.max(0,ent.limit-nowUsed)+nowTopup});
 }catch(error){console.error('AI assistant request failed',error);if(error?.code==='TIMEOUT')return send(res,504,{error:'The account check timed out. Please try again.',code:'ACCOUNT_CHECK_TIMEOUT'});return send(res,500,{error:'Unable to use conversational RightsRadar AI right now.',code:'AI_STATUS_FAILED'});}
};
