...
function setSessionCookie(res, token) {
  res.setHeader('Set-Cookie', `${COOKIE_NAME}=${encodeURIComponent(token)}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=${MAX_SESSION_AGE}`);
}
...
async function authRequest(req, path, payload) {
  ...
  const response = await fetch(`${base}${path}`, {
    ...
    headers: {
      ...
      'user-agent': String(req.headers['user-agent'] || 'RightsRadarUK/1.0')
    },
    body: JSON.stringify(payload)
  });
  const data = await response.json().catch(() => ({}));
  if (!response.ok) {
    const error = new Error(data.message || data.error?.message || data.error || 'Authentication request failed.');
...
async function getSessionUser(sql, token) {
  ...
  const rows = await sql`
    SELECT to_jsonb(s) AS session, to_jsonb(u) AS usr
    FROM neon_auth.session s
    JOIN neon_auth."user" u
      ON u.id = COALESCE(to_jsonb(s)->>'userId', to_jsonb(s)->>'user_id')
    WHERE to_jsonb(s)->>'token' = ${token}
    LIMIT 1
  `;
  ...
  return {
    id: String(user.id || ''),
    name: String(user.name || '').trim() || String(user.email || '').split('@')[0] || 'Member',
...
module.exports = async function handler(req, res) {
  ...
  try {
    if (action === 'sign-up') {
      ...
      const result = await authRequest(req, '/sign-up/email', { name, email, password });
      const token = result.token || result.session?.token;
      if (token) setSessionCookie(res, token);
      return send(res, 201, {
        ok: true,
        authenticated: Boolean(token),
        user: result.user ? { id: result.user.id, name: result.user.name, email: result.user.email } : null,
        message: token ? 'Account created and signed in.' : 'Account created. Check your email if verification is required before signing in.'
      });
    }
    ...
    if (action === 'sign-in') {
      ...
      const result = await authRequest(req, '/sign-in/email', { email, password, rememberMe: true });
      const token = result.token || result.session?.token;
      if (!token) return send(res, 401, { error: 'Sign-in did not return a valid session.' });
    ...
    if (action === 'sign-out') {
      ...
      return send(res, 200, { ok: true, authenticated: false, message: 'Signed out.' });
    }

    return send(res, 400, { error: 'Unknown account action.' });
  } catch (error) {
    if (error.message === 'AUTH_NOT_CONFIGURED') return send(res, 503, { error: 'Neon Auth is not connected to this deployment yet.' });
    console.error('auth action failed', error);
    const status = Number(error.status) || 500;
    const safeStatus = status >= 400 && status < 500 ? status : 500;
    return send(res, safeStatus, { error: safeStatus === 500 ? 'Unable to complete the account request right now.' : error.message });
  }
};
