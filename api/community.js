  ...
  if (ids.length) {
    comments = await sql`
      ...
      WHERE c.status = 'published'
        AND c.thread_id = ANY(${ids}::bigint[])
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'create-thread') {
      ...
      if (recent.length && Date.now() - new Date(recent[0].created_at).getTime() < 30000) {
        return send(res, 429, { error: 'Please wait a little before starting another conversation.' });
      }
      const inserted = await sql`
        INSERT INTO community_threads (author_user_id, author_name, title, body, category, jurisdiction)
        VALUES (${user.id}, ${user.name}, ${title}, ${text}, ${category}, ${jurisdiction})
        RETURNING id
      `;
      return send(res, 201, { ok: true, id: inserted[0]?.id, message: 'Conversation posted.' });
    }
    ...
    if (action === 'create-comment') {
      ...
      if (recent.length && Date.now() - new Date(recent[0].created_at).getTime() < 7000) {
        return send(res, 429, { error: 'Please wait a few seconds before commenting again.' });
      }
      await sql`
        INSERT INTO community_comments (thread_id, author_user_id, author_name, body)
        VALUES (${threadId}, ${user.id}, ${user.name}, ${text})
      `;
    ...
    if (action === 'report') {
      ...
      if ((!threadId && !commentId) || reason.length < 3 || reason.length > 500) {
        return send(res, 400, { error: 'Choose content to report and give a short reason.' });
      }
      ...
      if (!duplicate.length) {
        await sql`
          INSERT INTO community_reports (reporter_user_id, thread_id, comment_id, reason)
          VALUES (${user.id}, ${threadId}, ${commentId}, ${reason})
        `;
      }
